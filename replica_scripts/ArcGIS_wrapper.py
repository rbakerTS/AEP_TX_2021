...
class ArcGISFeature():
    ...
    def get_properties(self, layer_pos: int) -> arcgis.gis.PropertyMap:
        return self.l_flc.layers[layer_pos].properties

    def get_max_record_count(self, layer_pos) -> int:
        return self.get_properties(layer_pos).maxRecordCount

    def get_fields(self, layer_pos: int, just_field_names: bool = True) -> list:
        fields = self.get_properties(layer_pos).fields
        return [item.name for item in fields] if just_field_names else fields
